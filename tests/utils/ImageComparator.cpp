...
bool ImageComparator::compare(const std::string& image1_path, 
    ...
    if (error1) {
        std::cerr << "Error loading image1 (" << image1_path << "): " 
    ...
    if (error2) {
        std::cerr << "Error loading image2 (" << image2_path << "): " 
    ...
    if (width1 != width2 || height1 != height2) {
        std::cerr << "Image dimensions don't match: " 
                  << width1 << "x" << height1 << " vs " 
    ...
    if (diff_count > 0) {
        ...
        std::cerr << "Images differ: " << diff_count << " values (" 
                  << diff_percentage << "%) exceed tolerance " << tolerance 
                  << ", max diff: " << max_diff << std::endl;
        return false;
    }
    
    std::cout << "✅ Images match within tolerance " << tolerance << std::endl;
    return true;
}
...
bool ImageComparator::compareWithReference(const std::string& test_name,
    ...
    std::cout << "Comparing with reference: " << reference_path << std::endl;
